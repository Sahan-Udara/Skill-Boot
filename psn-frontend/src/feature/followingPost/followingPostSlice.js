          ...
          if (state.followingPosts !== null) {
              for (let i = 0; i < state.followingPosts.length; i++) {
                  if (state.followingPosts[i].post.id === action.payload.id) {
                      state.followingPosts[i].post.content = action.payload.content;
                      state.followingPosts[i].post.image = action.payload.image;
                      updatePostApi(action.payload);
                  }
              }
          }
      },
...
export const {addLove, addShare, addComment, updatePost, deletePost} = followingPostSlice.actions;
